...
//		model.addAttribute("pageMaker", new PageDTO(cri, 56));
	...
	public void withBfamily(@RequestParam("id") long id, Criteria cri, Model model) {
		...
		int total = service.getBfamilyListTotal(cri.getPageNum(), cri.getAmount(), id, cri.getType(), cri.getTypeArr(), cri.getKeyword()); 
		//int total = tmpList.size();
		
		model.addAttribute("list", service.getBfamilyList(cri.getPageNum(), cri.getAmount(), id, cri.getType(), cri.getTypeArr(), cri.getKeyword()));
